...
describe("mcp", () => {
  ...
  it("returns block kit card in tool response", async () => {
    ...
    assert.strictEqual(blocks[0].subtitle.text, "Engineer · :wave: Available");
    assert.ok(blocks[0].body.text.includes("test@example.com"));
    assert.ok(blocks[0].body.text.includes("Pacific Daylight Time"));
    assert.ok(blocks[0].icon.image_url.includes("slack-edge.com"));
  });

  it("returns install button block when not installed", async () => {
    const body = JSON.stringify({
      jsonrpc: "2.0",
      id: 3,
      method: "tools/call",
    ...
    assert.strictEqual(blocks[0].accessory.type, "button");
